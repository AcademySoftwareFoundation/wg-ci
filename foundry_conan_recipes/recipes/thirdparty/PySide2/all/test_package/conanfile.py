...
class TestPackage(ConanFile):
    ...
    def pythonVersion(self):
        return Version(self.deps_cpp_info["Python"].version)
    ...
    def libPaths(self):
        if self.settings.os == 'Linux' or self.settings.os == 'Macos':
            return [
                *self.deps_cpp_info["PySide2"].lib_paths,
                *self.deps_cpp_info["Qt"].lib_paths,
            ]
        if self.settings.os == 'Windows':
            return [
                *self.deps_cpp_info["PySide2"].bin_paths,
    ...
    def sitePkgPath(self):
        basePath = self.deps_cpp_info["PySide2"].lib_paths[0]
        if self.settings.os == 'Linux' or self.settings.os == 'Macos':
            return path.join(
                basePath,
    ...
    def test(self):
        ...
        cmake.definitions["CONAN_PYSIDE_VERSION"] = str(self.deps_cpp_info["PySide2"].version)
